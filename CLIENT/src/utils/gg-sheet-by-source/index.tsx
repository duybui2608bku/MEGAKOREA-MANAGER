...
const ggsshetBySource = async (dateOrDates: string | string[]): Promise<SheetRecord[]> => {
  const sheetUrl =
  ...
  try {
    ...
    const groupedData = _.groupBy(filteredData, (item) => `${item.branch}|${item.source}`)
...
export default ggsshetBySource
